...
use atspi::{Event, ObjectRef, State, WindowEvents};
...
fn recursive_collect_children<'a>(
    proxy: &'a AccessibleProxy<'a>,
    conn: &'a Connection,
) -> std::pin::Pin<
    Box<dyn std::future::Future<Output = Result<Vec<ObjectRef>, Box<dyn Error>>> + Send + 'a>,
> {
    Box::pin(async move {
        ...
        for child in proxy.get_children().await?.iter() {
            ...
            collected_refs.push(child.clone());

            
            let state = block_on(child_proxy.get_state())?;

            let is_link = state.contains(State::Showing);

            let mut child_refs = recursive_collect_children(&child_proxy, conn).await?;
            if is_link {
                collected_refs.append(&mut child_refs);
            }
        }
    ...
    eframe::run_native(
        ...
        Box::new({
            ...
            move |cc| {
                ...
                tokio::spawn(async move {
                    atspi_clone.register_event::<WindowEvents>().await.unwrap();
                    ...
                    while let Some(event) = events.next().await {
                        match event {
                            Ok(Event::Window(WindowEvents::Activate(ev))) => {
                                ...
                                tokio::spawn(async move {
                                    let a11y_proxy =
                                        ev.item.into_accessible_proxy(&conn_inner).await;
                                    match a11y_proxy {
                                        Ok(proxy) => {
                                            match recursive_collect_children(&proxy, &conn_inner)
                                                .await
                                            {
                                                Ok(object_refs) => {
                                                    if let Err(err) = tx_inner.send(object_refs) {
                                                        eprintln!(
                                                            "Error sending object refs: {err}"
                                                        );
                                                    }
                                                }
            ...
            .show(ctx, |ui| {
                if let Some(state) = &self.state {
                    ...
                    state.iter().for_each(|point| {
                        let a11y_proxy = block_on(point.as_accessible_proxy(&self.conn));
                        match a11y_proxy {
                            Ok(proxy) => {
                                if let Ok(proxies) = block_on(proxy.proxies()) {
                                    if let Ok(component) = block_on(proxies.component()) {
                                        if let Ok((x0, y0, _, _)) = block_on(
                                            ...
                                            eprintln!(
                                                "Error: Failed to get extents from component"
                                            );
                                        }
                                    } else {
                                        eprintln!("Error: Failed to get component from proxies");
                                    }
                                } else {
                                    eprintln!("Error: Failed to get proxies from proxy");
                                }
                            }
                            Err(err) => println!("Error creating proxy: {err}"),
                        }
                    });
                }
            });
